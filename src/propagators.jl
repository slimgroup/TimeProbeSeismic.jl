...
function backprop(model::PyObject, y::Array{Float32}, rcv_coords::Array{Float32},
    ...
    # Create operator and run
    subs = model.spacing_map
    op = dv.Operator(vcat(pde, geom_expr, probe_eq), subs=subs,
                     name="adjointp", opt=ut.opt_op(model))
...
function born(model::PyObject, src_coords::Array{Float32}, rcv_coords::Array{Float32},
              wavelet::Array{Float32}, e::Array{Float32}, nlind::Bool=false,
    ...
    # Create operator and run
    subs = model.spacing_map
    op = dv.Operator(vcat(pde, pdel, geom_expr, geom_exprl, probe_eq), subs=subs,
    ...
    # Output
    rd = nlind ? rcv.data : 0
    return rd, rcvl.data, eu, summary
end
...
function born_with_back(model::PyObject, src_coords::Array{Float32}, rcv_coords::Array{Float32},
    ...
    # Create operator and run
    subs = model.spacing_map
    op = dv.Operator(vcat(pde, pdel, pdev, geom_exprl, geom_exprv, probe_eq),
...
function time_probe(e::Array{Float32, 2}, wf::PyObject, model::PyObject; fw=true,
                    isic=false, pe=nothing)
...
function combine(eu::PyObject, ev::PyObject, model::PyObject, isic::Bool=false)
    ...
    op = dv.Operator(dv.Eq(g, g - eq), subs=eu.grid.spacing_map, name="traceprobes",
                     opt=ut.opt_op(model))
    op()
    g
end