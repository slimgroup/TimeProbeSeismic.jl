...
function forward(model::Model, q::judiVector, dobs::judiVector; options=Options(), ps=16, modelPy=nothing)
    ...
    q_data = time_resample_data(q[1], dt_Comp)
    d_data = time_resample_data(dobs[1], dt_Comp)
    ...
    rec = time_resample_data(rec, dobs.geometry.dt[1], dt_Comp, dobs.geometry.t[1])
...
function born(model::Model, q::judiVector, dobs::judiVector, dm; options=Options(), ps=16, modelPy=nothing)
    ...
    recnl = time_resample_data(recnl, dobs.geometry.dt[1], dt_Comp, dobs.geometry.t[1])
    recl = time_resample_data(recl, dobs.geometry.dt[1], dt_Comp, dobs.geometry.t[1])
...
function backprop(model::Model, residual::judiVector, Q::Array{Float32}, eu::PyObject;
    ...
    isnothing(modelPy) && (modelPy = devito_model(model, options))
    # Interpolate input data to computational grid
    d_data = time_resample_data(residual[1], dt_Comp)
