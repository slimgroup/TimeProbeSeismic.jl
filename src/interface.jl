...
function born(model::Model, q::judiVector, dobs::judiVector, dm; options=Options(),
              ps=16, modelPy=nothing, nlind=false)
    # Python model
    ...
    recnl, recl, eu, _ = born(modelPy, src_coords, rec_coords, q_data, Q, nlind, options.space_order, options.isic)
    recnl = nlind ? time_resample_data(recnl, dobs.geometry.dt[1], get_dt(model), dobs.geometry.t[1]) : 0
    recnl = nlind ? judiVector(dobs.geometry, recnl) : 0
    recl = time_resample_data(recl, dobs.geometry.dt[1], get_dt(model), dobs.geometry.t[1])
    return recnl, judiVector(dobs.geometry, recl), Q, eu
end
...
function born_with_back(model::Model, q::judiVector, dobs::judiVector, dm; options=Options(), ps=16, modelPy=nothing)
    ...
    recl = time_resample_data(recl, dobs.geometry.dt[1], get_dt(model), dobs.geometry.t[1])
    g = combine(eu, ev, modelPy, options.isic)
