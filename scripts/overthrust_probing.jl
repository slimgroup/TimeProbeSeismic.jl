...
recGeometry = Geometry(xrec, yrec, zrec; dt=dtD, t=timeD, nsrc=1)

# Set up source geometry (cell array with source locations for each shot)
xsrc = convertToCell(range(0f0, (n[1] - 1)*d[1]/2, length=nsrc))[2]
ysrc = convertToCell(range(0f0, 0f0, length=nsrc))[2]
zsrc = convertToCell(range(12.5f0, 12.5f0, length=nsrc))[2]
...
for i=1:1
    ...
    tight_layout()
    savefig("Probed_grad$inlinespacing.png", bbox_inches="tight")
    ...
    tight_layout()
    savefig("Probed_err$i.png", bbox_inches="tight")
end
...
semilogx([2^i for i=1:8], similar1, "-ob", label="Full gradient reflections", basex=2)
...
legend()
savefig("simil.png", bbox_inches="tight")
...
savefig("adjtest.png", bbox_inches="tight")
...
tight_layout()
savefig("Zi.png", bbox_inches="tight")
